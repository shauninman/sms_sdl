...
void Sound_Update(int16_t* restrict sound_buffer, unsigned long len)
{
	sdl_write_buffer((uint8_t*)sound_buffer, len * 4);
}

uint32_t Sound_Underrun_Likely() {
	if (buffered_bytes < BUFFSIZE * UNDERRUN_THRESHOLD) return 1; else return 0;
}
